...
const Home = () => {
  ...
  const { newsdata, status } = useSelector(state => state.newsapi)
      ...
      <div className="container">
        <div className="row" >
          <div className="col" id='btncol'>
            ...
            <Link to='craeteblog'><button type="button" class="btn btn-primary mt-3">
              new Post
            </button></Link>
          </div>
        </div>
        <div className="row mt-5 rowmain" >
          {
            newsdatas && newsdatas.map((ele, id) => {
              return <>

                <div className="col-6 col-md-4" key={ id } id='newitem'>
